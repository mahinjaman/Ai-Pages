...
interface ViewportProps {
  [key: string]: { width: number | string; icon: SpriteIconProps };
}
...
const viewportData: ViewportProps = {
...
// const typographyOptions: TypographyListProps[] = [
...
function ScreenButton({ icon, screen, screenSize, onScreenChange }: ScreenButtonProps) {
...
export default function Simulator({ src, defaultHeight }: Props) {
  ...
  const setIframeHeight = (type?: string) => {
    ...
    >
      ...
      >
        <Stack direction="row" sx={{ display: { xs: 'none', md: 'flex' }, gap: 1 }}>
          {Object.keys(viewportData).map((item: string, index: number) => (
            <Fragment key={index}>
